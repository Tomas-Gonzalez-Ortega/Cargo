...
public class CargoTester {
    ...
    public static void main(String[] args) {
        ...
        testRemove();
        testGetItems_name();
        testGetHeaviest_Average();
        testGetHeaviest_Average_name();
        testGetItems_tracking();
        testGetItemsMap();
    ...
    private static void testMakeItem() {
        ...
        assert "Item1".equals(item.name);
        assert 10 == item.weight;
        assert 101 == item.tracking;

        item = new Cargo.Item("Item2", 20);
        assert "Item2".equals(item.name);
        assert 20 == item.weight;
        assert 102 == item.tracking;
    }
    ...
    private static void testAddItems() {
        ...
        assert cb.contains(item.tracking);
    ...
    private static void testRemove() {
        ...
        cb.add(item);
        assert cb.remove(item.tracking);
        assert 0 == cb.getItemCount();
        assert !cb.isOverWeight();
        assert 0 == cb.getTotalWeight();
        assert !cb.contains(item.tracking);
        ...
        assert 3 == cb.getItemCount();
        assert cb.isOverWeight();
        assert cb.remove(items[1].tracking);    // typical, remove 1 item
        assert 2 == cb.getItemCount();
        assert !cb.isOverWeight();
        assert cb.contains(items[0].tracking);
        assert !cb.contains(items[1].tracking);
        assert cb.contains(items[2].tracking);
        // typical, remove what's left
        assert cb.remove(items[0].tracking);
        assert !cb.remove(items[1].tracking);
        assert cb.remove(items[2].tracking);
        // check
        assert 0 == cb.getItemCount();
        assert 0 == cb.getTotalWeight();
        assert !cb.isOverWeight();
    ...
    private static void testGetItems_tracking() {
        ...
        HashSet<Cargo.Item> sub;
        // boundary, at ends
        sub = cb.getItems(items[0].tracking, items[3].tracking);
        assert 2 == sub.size();
        assert sub.contains(items[0]);
        assert sub.contains(items[3]);

        // typical, in middle
        sub = cb.getItems(items[1].tracking, items[2].tracking);
        assert 2 == sub.size();
    ...
    private static void testGetItemsMap() {
        ...
        HashMap<String, HashSet<Cargo.Item>> map;
        HashSet<Cargo.Item> sub;
